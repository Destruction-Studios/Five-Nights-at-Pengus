...
func _process(delta: float) -> void:
	$Fade.visible = !is_active
	
	var in_range = 0

	if is_active:
		for i in bars.size():
			var current_progress = progresses[i]
			var bar = bars[i]
			var acceptable_range = acceptable_ranges[i]
...
func create_range() -> Array[float]:
	var max_r = randf_range(GameSettings.FILL_RANGE + randf_range(5.0, 7.5),
		100.0 - randf_range(12.5, 15.0))
